...
function toggleStyle(mode) {
    ...
    let btnStyle = mode === 'night' ? 'inverted' : 'basic';
    const btns = document.querySelectorAll('.ui.button');
    btns.forEach(b => {
        let color = b.className.split(' ')[1];
...
countbtn.addEventListener('click', (e) => {
    ...
    let time = countdowninput.value * 60;
...
Timer.prototype.count = function () {
    const now = Date.now();
    let then = now + this.countdown * 1000;
...
class View {
    ...
    edit(i) {
        ...
        editing.innerHTML =
            `<input id='edit'><button class="ui pink basic button" id='save'>save</button><button class="ui grey basic button" id='cancel'>cancel</button>`
    ...
    render() {
        ...
        let html = '';
        this.lists.map((li, index) => {
            html +=
                `<li  data-index=${index}><input type="checkbox" ${li.done ? 'checked' : ''} class="item"><span style="text-decoration:${li.done? 'line-through':''}" >${li.text}</span><a class="x"></a></li>`
        }).join('')
